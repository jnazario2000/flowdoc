...
export default function EditorPage() {
  ...
  const saveDocument = useCallback(async () => {
    if (!hasUnsavedChanges || !documentContent) return
  ...
  const goBackToRepository = () => {
    if (repoKey) navigate('/repositorypage', { state: { repoKey, repoInfo, token } })
    else navigate('/')
  }
          ...
          {selectedCodeFile && (
            <div style={{ borderTop: '1px solid #ddd', padding: '0.75rem 1rem', backgroundColor: '#f9f9f9', fontSize: '0.85em' }}>
              ...
              {showAllAnchors && getCurrentFileAnchors().length > 0 ? (
                ...
                <p style={{ margin: '0.5rem 0 0 0', opacity: 0.7 }}>Click line numbers to select (Shift+Click for range)</p>
              )}
            </div>
          )}
        </div>
